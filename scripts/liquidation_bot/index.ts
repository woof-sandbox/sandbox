...
import { arbitragePurchaseableCollateral, liquidateUnderwaterBorrowers, getAssets, Asset } from "./liquidateUnderwaterBorrowers";
...
async function main() {
  ...
  googleCloudLog(LogSeverity.INFO, `Liquidation Bot started ${JSON.stringify({ network, deployment, liquidatorAddress, useFlashbots })}`);
  ...
  const liquidator = (await hre.ethers.getContractAt("OnChainLiquidator", liquidatorAddress, signer)) as OnChainLiquidator;
  ...
  while (true) {
    ...
    if (currentBlockNumber !== lastBlockNumber) {
      ...
      const liquidationAttempted = await liquidateUnderwaterBorrowers(comet, liquidator, signerWithFlashbots, network, deployment);
      if (!liquidationAttempted) {
        await arbitragePurchaseableCollateral(comet, liquidator, assets, signerWithFlashbots, network, deployment);
      }
    } else {
