...
async function executeCrossChainProposalActions(dm) {
  ...
  // Update supply cap
  await setNextBaseFeeToZero(dm);
  await configurator.connect(timelock).setConfiguration(comet.address, configuration, { gasPrice: 0 });

  // Upgrade comet
  await setNextBaseFeeToZero(dm);
  await cometAdmin.connect(timelock).deployAndUpgradeTo(configurator.address, comet.address, { gasPrice: 0 });
}
