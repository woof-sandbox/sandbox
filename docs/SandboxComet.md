...
```solidity
function quoteCollateral(address asset, uint256 baseAmount) public view returns (uint256 amountOut, uint256 reserveFee, uint256 protocolFee, uint256 controllerFee)
```
...
```solidity
function _distributeProfit(uint256 profitAmount) internal view returns (uint256 _reserveFee, uint256 _protocolFee, uint256 _controllerFee)
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _reserveFee | uint256 | Fee on profit in favour of reserves |
| _protocolFee | uint256 | Fee on profit in favour of DAO |
