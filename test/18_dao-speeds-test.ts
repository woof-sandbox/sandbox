...
import { ethers, exp, expect, fastForward, makeProtocol, SnapshotRestorer, takeSnapshot } from "./helper/helpers";
...
describe("18. DAO speeds", function() {
    ...
    describe("base tracking speed setting", function() {
        ...
        it("should emit an event when dao base speeds are set", async function() {
            ...
            await expect(comet.connect(dao).setDaoBaseSpeeds(newSpeed, newSpeed))
                .to.emit(comet, "DaoSpeedsChanged")
                .withArgs(newSpeed, newSpeed);
        });

        it("should revert if non-dao tries to set dao base speeds", async function() {
            const newSpeed = exp(1, 6);

            await expect(comet.connect(alice).setDaoBaseSpeeds(newSpeed, newSpeed)).to.be.revertedWithCustomError(
                comet,
                "Unauthorized"
            );
        });
