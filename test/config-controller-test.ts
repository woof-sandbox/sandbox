...
describe("ConfigController", () => {
    describe("Create Market", () => {
        it("should create a comet", async () => {
            ...
            const cometContract: ISandboxComet = <ISandboxComet>(
                await ethers.getContractAt("ISandboxComet", cometAddress)
            );
            ...
            expect(await cometContract.supplyPerSecondInterestRateSlopeLow()).to.equal(
                curve.supplyPerYearInterestRateSlopeLow.div(secondsInYear)
            );
            expect(await cometContract.supplyPerSecondInterestRateSlopeHigh()).to.equal(
                curve.supplyPerYearInterestRateSlopeHigh.div(secondsInYear)
            );
            expect(await cometContract.supplyPerSecondInterestRateBase()).to.equal(
                curve.supplyPerYearInterestRateBase.div(secondsInYear)
            );
            expect(await cometContract.borrowKink()).to.equal(curve.borrowKink);
            expect(await cometContract.borrowPerSecondInterestRateSlopeLow()).to.equal(
                curve.borrowPerYearInterestRateSlopeLow.div(secondsInYear)
            );
            expect(await cometContract.borrowPerSecondInterestRateSlopeHigh()).to.equal(
                curve.borrowPerYearInterestRateSlopeHigh.div(secondsInYear)
            );
            expect(await cometContract.borrowPerSecondInterestRateBase()).to.equal(
                curve.borrowPerYearInterestRateBase.div(secondsInYear)
            );

            // -- Collaterals tokens --
            expect(await cometContract.numAssets()).to.eq(Object.keys(tokens).length - 1);
            const collateralTokens: CollateralTokenConfigStruct[] = (await cometContract.getConfiguration())
                .assetConfigs;
            // -- check whitelisted collaterals --
            const whitelistedCollateralAddresses = Array.from(collateralTokens, (token) => token.collateralToken);
            ...
            for (let token in tokens) {
                ...
                if (token !== baseTokenSymbol && token !== unsupportedTokenSymbol) {
                    ...
                    expect(
                        whitelistedCollateralAddresses.filter((addr) => addr === tokens[token].address).length
                    ).to.equal(1);
