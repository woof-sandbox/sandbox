...
import { Fauceteer, Fauceteer__factory, FaucetToken__factory } from "../build/types";
...
describe.skip("Fauceteer", function () {
  ...
  it("throws an error if balance of asset is 0", async () => {
    ...
    await expect(fauceteer.connect(requester).drip(USDC.address)).to.be.revertedWith("custom error 'BalanceTooLow()'");
  });
  ...
  it("limits each address to one request per asset per day", async () => {
    ...
    await expect(fauceteer.connect(r1).drip(USDC.address)).to.be.revertedWith("custom error 'RequestedTooFrequently()'");
    await expect(fauceteer.connect(r1).drip(COMP.address)).to.be.revertedWith("custom error 'RequestedTooFrequently()'");
