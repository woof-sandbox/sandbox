...
import { ContractReceipt, ContractTransaction, Event, BigNumber } from "ethers";
...
describe("38. Create Change Curve Proposal", () => {
    ...
    describe("accept ProposeCometCurve proposal - reverts", () => {
        ...
        it("should revert when the comet is not belongs to this config controller", async () => {
            ...
            /// Try to create a new proposal for change curve from the first controller after the transfer (should fail with UnknownComet).
            await expect(configController.connect(owner).createProposal(calldata, ethers.Wallet.createRandom().address, proposalType)).to.be.revertedWithCustomError(configController, "UnknownComet");
        });
    });
