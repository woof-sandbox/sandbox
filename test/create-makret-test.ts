...
import { IMarket } from '../build/types';
...
describe('Create Market', () => {
    it('should create a market', async () => { 
        ...
        expect((await marketContract.getMarketConfig()).baseToken).to.eq(tokens[await baseToken.symbol()].address);
        expect((await marketContract.getMarketConfig()).priceFeed).to.eq(priceFeeds[await baseToken.symbol()].address);
        expect((await marketContract.getMarketConfig()).collateraTokens.length).to.eq(Object.keys(tokens).length - 1);

        for (let token in tokens) {
            if (token != await baseToken.symbol()) {
                const collateralTokenConfig = (await marketContract.getMarketConfig()).collateraTokens.find(
                    (collateralToken) => collateralToken.collateralToken === tokens[token].address
                );
                expect(collateralTokenConfig).to.not.be.undefined;
        ...
        const collateralTokens = (await marketContract.getMarketConfig()).collateraTokens;
