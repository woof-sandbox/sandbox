...
describe("1. System Initialization", function () {
  ...
  describe("Sandbox Controller deployment", function () {
    ...
    it("reverts if admin = 0", async function () {
      ...
      await expect(_SandboxControllerFactory.connect(dao).deploy(...Object.values(opts))).to.be.revertedWithCustomError(
    ...
    it("reverts if dao = 0", async function () {
      ...
      await expect(_SandboxControllerFactory.deploy(...Object.values(opts))).to.be.revertedWithCustomError(
        _SandboxControllerFactory,
        "ZeroAddress"
      );
    });
