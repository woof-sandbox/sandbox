...
import { SimplePriceFeed__factory, SimpleWstETH__factory, WstETHPriceFeed__factory } from "../../build/types";
...
export async function makeWstETH({ stEthPrice, tokensPerStEth }) {
  ...
  const wstETHPriceFeed = await wstETHPriceFeedFactory.deploy(stETHPriceFeed.address, simpleWstETH.address, 8);
