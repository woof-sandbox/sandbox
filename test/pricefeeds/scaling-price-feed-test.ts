...
import { SimplePriceFeed__factory, ScalingPriceFeed__factory } from "../../build/types";
...
describe.skip("scaling price feed", function () {
  it(`description is set properly`, async () => {
    const { simplePriceFeed, scalingPriceFeed } = await makeScalingPriceFeed({
      price: exp(10, 18),
      priceFeedDecimals: 18,
    });
  ...
  describe("latestRoundData", function () {
    ...
    it("passes along roundId, startedAt, updatedAt and answeredInRound values from underlying price feed", async () => {
      ...
      const { roundId, startedAt, updatedAt, answeredInRound } = await scalingPriceFeed.latestRoundData();
