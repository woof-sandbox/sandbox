...
describe("SandboxController", function() {
    ...
    describe("whitelistBaseAsset", function() {
        ...
        it("reverts if caller is not owner or dao", async function() {
            ...
            await expect(
                sandboxController
                    .connect(attacker)
                    .whitelistBaseAsset(token.address, priceFeed.address, makeValidCurve(), 10)
        ...
        it("reverts if feed not a valid aggregator", async function() {
            ...
            await expect(sandboxController.whitelistBaseAsset(token.address, badFeed.address, makeValidCurve(), 10)).to
                .be.reverted;
        });
        ...
        it("owner can do it, dao can do it", async function() {
            ...
            await sandboxController
                .connect(owner)
                .whitelistBaseAsset(token1.address, feed1.address, makeValidCurve(), 100);
            const token2 = await makeMockERC20({ name: "T8", symbol: "T8" });
            const feed2 = await makePriceFeed({}, token2.address);
            await sandboxController
                .connect(dao)
                .whitelistBaseAsset(token2.address, feed2.address, makeValidCurve(), 200);
    ...
    describe("whitelistCollateralAsset", function() {
        ...
        it("reverts if caller is not authorized", async function() {
            ...
            await expect(
                sandboxController
                    .connect(attacker)
                    .whitelistCollateralAsset(token.address, priceFeed.address, 8000, 5000, 6000, 9000, 7000, 9500)
            ).to.be.revertedWithCustomError(sandboxController, "Unauthorized");
        });
        ...
        it("reverts if any factor is zero", async function() {
            ...
            await expect(
                sandboxController.whitelistCollateralAsset(token.address, feed.address, 0, 8000, 6000, 9000, 7000, 9500)
        ...
        it("owner can do it, dao can do it", async function() {
            ...
            await sandboxController
                .connect(owner)
                .whitelistCollateralAsset(token1.address, feed1.address, 4000, 9000, 5000, 9500, 7000, 9000);

            const token2 = await makeMockERC20({ name: "C8", symbol: "C8" });
            const feed2 = await makePriceFeed({}, token2.address);
            await sandboxController
                .connect(dao)
                .whitelistCollateralAsset(token2.address, feed2.address, 4000, 9000, 5000, 9500, 7000, 9000);
    ...
    describe("setConfiguration", function() {
        ...
        let configParams: SandboxControllerConfigurationStruct;
        ...
        beforeEach(async function() {
            ...
            configParams = {
                storeFrontPriceFactor: parseEther("0.4").toString(),
                minUpdateTime: BigNumber.from(500),
                suggestedAmountOfSeedReserves: "2000",
                suggestedLockTimeOfSeedReserves: 2000,
                targetPercent: ethers.utils.parseEther("0.5").toString(),
                maxUpdateTime: BigNumber.from(600),
            };
            const c = await makeSandboxController(opts);
            sandboxController = c.sandboxController as SandboxController;
        });

        it("reverts if caller is not owner", async function() {
            await expect(sandboxController.connect(dao).setConfiguration(configParams)).to.be.revertedWithCustomError(
                sandboxController,
                "NotOwner"
            );
            await expect(
                sandboxController.connect(attacker).setConfiguration(configParams)
            ).to.be.revertedWithCustomError(sandboxController, "NotOwner");
        });

        it("reverts if storeFrontPriceFactor >= 1e18", async function() {
            configParams.storeFrontPriceFactor = parseEther("1").toString();
            await expect(sandboxController.setConfiguration(configParams)).to.be.revertedWithCustomError(
                sandboxController,
                "InvalidFactors"
            );
        });
        ...
        it("reverts if minUpdateTime is greater than maxUpdateTime", async function() {
            configParams.maxUpdateTime = 10;
            await expect(sandboxController.setConfiguration(configParams)).to.be.revertedWithCustomError(
                sandboxController,
                "InvalidFactors"
            );
        ...
        it("reverts if suggestedAmountOfSeedReserves=0", async function() {
            configParams.suggestedAmountOfSeedReserves = "0";
            await expect(sandboxController.setConfiguration(configParams)).to.be.revertedWithCustomError(
                sandboxController,
                "InvalidFactors"
            );
        ...
        it("reverts if suggestedLockTimeOfSeedReserves=0", async function() {
            configParams.suggestedLockTimeOfSeedReserves = 0;
            await expect(sandboxController.setConfiguration(configParams)).to.be.revertedWithCustomError(
                sandboxController,
                "InvalidFactors"
            );
        ...
        it("updates configuration with valid values and emits event", async function() {
            const tx = await sandboxController.setConfiguration(configParams);
            expect((await sandboxController.controllerConfiguration()).storeFrontPriceFactor).to.equal(
                configParams.storeFrontPriceFactor
            );
            expect((await sandboxController.controllerConfiguration()).minUpdateTime).to.equal(
                configParams.minUpdateTime
            );
            expect((await sandboxController.controllerConfiguration()).suggestedAmountOfSeedReserves).to.equal(
                configParams.suggestedAmountOfSeedReserves
            );
            expect((await sandboxController.controllerConfiguration()).suggestedLockTimeOfSeedReserves).to.equal(
                configParams.suggestedLockTimeOfSeedReserves
            );
    ...
    describe("setFeeEnabled", function() {
        ...
        it("reverts if caller is not owner", async function() {
            await expect(sandboxController.connect(owner).setFeeEnabled(true)).to.be.revertedWithCustomError(
                sandboxController,
                "NotDao"
            );
            await expect(sandboxController.connect(attacker).setFeeEnabled(true)).to.be.revertedWithCustomError(
                sandboxController,
                "NotDao"
            );
        });
    ...
    describe("addBaseAssetCurve", function() {
        ...
        it("works if called by owner or dao, checks that a new curve is appended, checks BaseAssetCurveAdded event", async function() {
            ...
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateSlopeLow).to.equal(
                newCurve.supplyPerYearInterestRateSlopeLow
            );
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateSlopeHigh).to.equal(
                newCurve.supplyPerYearInterestRateSlopeHigh
            );
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateBase).to.equal(
                newCurve.supplyPerYearInterestRateBase
            );
            expect(ev.args.baseAssetCurve.borrowKink).to.equal(newCurve.borrowKink);
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateSlopeLow).to.equal(
                newCurve.borrowPerYearInterestRateSlopeLow
            );
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateSlopeHigh).to.equal(
                newCurve.borrowPerYearInterestRateSlopeHigh
            );
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateBase).to.equal(
                newCurve.borrowPerYearInterestRateBase
            );
            ...
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateSlopeLow).to.equal(
                anotherCurve.supplyPerYearInterestRateSlopeLow
            );
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateSlopeHigh).to.equal(
                anotherCurve.supplyPerYearInterestRateSlopeHigh
            );
            expect(ev.args.baseAssetCurve.supplyPerYearInterestRateBase).to.equal(
                anotherCurve.supplyPerYearInterestRateBase
            );
            expect(ev.args.baseAssetCurve.borrowKink).to.equal(anotherCurve.borrowKink);
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateSlopeLow).to.equal(
                anotherCurve.borrowPerYearInterestRateSlopeLow
            );
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateSlopeHigh).to.equal(
                anotherCurve.borrowPerYearInterestRateSlopeHigh
            );
            expect(ev.args.baseAssetCurve.borrowPerYearInterestRateBase).to.equal(
                anotherCurve.borrowPerYearInterestRateBase
            );
            ...
            expect(curves[2].supplyPerYearInterestRateSlopeLow).to.equal(
                anotherCurve.supplyPerYearInterestRateSlopeLow
            );
            expect(curves[2].supplyPerYearInterestRateSlopeHigh).to.equal(
                anotherCurve.supplyPerYearInterestRateSlopeHigh
            );
            ...
            expect(curves[2].borrowPerYearInterestRateSlopeLow).to.equal(
                anotherCurve.borrowPerYearInterestRateSlopeLow
            );
            expect(curves[2].borrowPerYearInterestRateSlopeHigh).to.equal(
                anotherCurve.borrowPerYearInterestRateSlopeHigh
            );
    ...
    describe("changeBaseAssetCurve", function() {
        ...
        it("works if dao calls it, checks old vs new curves and emits BaseAssetCurveChanged", async function() {
            ...
            expect(ev.args.oldCurve.supplyPerYearInterestRateSlopeLow).to.equal(
                oldCurve.supplyPerYearInterestRateSlopeLow
            );
            expect(ev.args.oldCurve.supplyPerYearInterestRateSlopeHigh).to.equal(
                oldCurve.supplyPerYearInterestRateSlopeHigh
            );
            ...
            expect(ev.args.oldCurve.borrowPerYearInterestRateSlopeLow).to.equal(
                oldCurve.borrowPerYearInterestRateSlopeLow
            );
            expect(ev.args.oldCurve.borrowPerYearInterestRateSlopeHigh).to.equal(
                oldCurve.borrowPerYearInterestRateSlopeHigh
            );
            ...
            expect(ev.args.newCurve.supplyPerYearInterestRateSlopeLow).to.equal(
                newCurve.supplyPerYearInterestRateSlopeLow
            );
            expect(ev.args.newCurve.supplyPerYearInterestRateSlopeHigh).to.equal(
                newCurve.supplyPerYearInterestRateSlopeHigh
            );
            ...
            expect(ev.args.newCurve.borrowPerYearInterestRateSlopeLow).to.equal(
                newCurve.borrowPerYearInterestRateSlopeLow
            );
            expect(ev.args.newCurve.borrowPerYearInterestRateSlopeHigh).to.equal(
                newCurve.borrowPerYearInterestRateSlopeHigh
            );
            ...
            expect(updatedCurve.supplyPerYearInterestRateSlopeHigh).to.equal(
                newCurve.supplyPerYearInterestRateSlopeHigh
            );
            ...
            expect(updatedCurve.borrowPerYearInterestRateSlopeHigh).to.equal(
                newCurve.borrowPerYearInterestRateSlopeHigh
            );
    ...
    describe("transferOwner", function() {
        ...
        it("reverts if caller not owner", async function() {
            await expect(sandboxController.connect(dao).transferOwner(dao.address)).to.be.revertedWithCustomError(
                sandboxController,
                "NotOwner"
            );
            await expect(
    ...
    describe("transferDao", function() {
        ...
        it("reverts if caller not owner", async function() {
            await expect(sandboxController.connect(owner).transferDao(other.address)).to.be.revertedWithCustomError(
                sandboxController,
                "NotDao"
            );
            await expect(
