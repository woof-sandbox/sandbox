...
describe("14. updateAssetsIn", function () {
  it("adds asset to user's asset list when initialUserBalance = 0 and finalUserBalance > 0", async () => {
    ...
    expect(await comet.getAssetList(user.address)).to.deep.equal([compAddress, wethAddress, wbtcAddress]);
  });
  ...
  it("reverts for non-existent asset address", async () => {
    ...
    await expect(comet.updateAssetsInExternal(user.address, randomAddress, 0, 100)).to.be.revertedWith("custom error 'BadAsset()'");
  });
});
