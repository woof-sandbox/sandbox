...
async function setupContracts(cache: Cache, hre: HardhatRuntimeEnvironment): Promise<TestContracts> {
  let proxyAdmin: ProxyAdmin = await deploy("vendor/proxy/transparent/ProxyAdmin.sol", [], hre, {
    cache,
    network: "test-network",
  });

  let finnImpl: Dog = await deploy("test/Dog.sol", ["finn:implementation", "0x0000000000000000000000000000000000000000", []], hre, {
    cache,
    network: "test-network",
  });
  ...
  let molly: Dog = await deploy("test/Dog.sol", ["molly", proxy.address, []], hre, { cache, network: "test-network" });

  let spot: Dog = await deploy("test/Dog.sol", ["spot", proxy.address, []], hre, { cache, network: "test-network" });
