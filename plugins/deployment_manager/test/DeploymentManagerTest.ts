...
import { Dog, ProxyAdmin, TransparentUpgradeableProxy } from "../../../build/types";
...
export async function setupContracts(deploymentManager: DeploymentManager): Promise<TestContracts> {
  let proxyAdminArgs: [string] = [await (await deploymentManager.getSigners())[0].getAddress()];
  ...
  let finnImpl: Dog = await deploymentManager.deploy("finnImpl", "test/Dog.sol", [
    "finn:implementation",
  ...
  let proxy: TransparentUpgradeableProxy = await deploymentManager.deploy(
    ...
    [finnImpl.address, proxyAdmin.address, (await finnImpl.populateTransaction.initializeDog("finn", finnImpl.address, [])).data]
  );

  let molly: Dog = await deploymentManager.deploy("molly", "test/Dog.sol", ["molly", proxy.address, []]);

  let spot: Dog = await deploymentManager.deploy("spot", "test/Dog.sol", ["spot", proxy.address, []]);
...
describe("DeploymentManager", () => {
  ...
  describe("import", () => {
    ...
    it.skip("should import succesfully", async () => {
      ...
      let importResult = await deploymentManager.import("0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e");
  ...
  describe("deploy", () => {
    it("should deploy succesfully", async () => {
      ...
      let spot: Dog = await deploymentManager.deploy("spot", "test/Dog.sol", ["spot", "0x0000000000000000000000000000000000000000", []]);
  ...
  describe("verifyContracts", () => {
    it("should verify contracts successfully", async () => {
      ...
      await putVerifyArgs(deploymentManager.cache, token.address, verifyArgs);
  ...
  describe("putAlias", () => {
    it("should update contract cache", async () => {
      ...
      let spot: Dog = await deploymentManager.deploy("spot", "test/Dog.sol", ["spot", "0x0000000000000000000000000000000000000000", []]);
      let molly: Dog = await deploymentManager.deploy("molly", "test/Dog.sol", ["molly", "0x0000000000000000000000000000000000000000", []]);
  ...
  describe("spider", () => {
    it("should spider succesfully", async () => {
      ...
      let { finn } = await setupContracts(deploymentManager);
  ...
  describe("generateMigration", () => {
    it("should generate expected migration", async () => {
      ...
      expect(await deploymentManager.cache.readCache({ rel: ["migrations", "1_cool.ts"] })).to.equal(expectedTemplate);
    });
  });
