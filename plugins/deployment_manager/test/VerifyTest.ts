...
describe("Verify", () => {
  describe("via artifacts", () => {
    it("verify from artifacts [success]", async () => {
      ...
      await verifyContract({ via: "artifacts", address: token.address, constructorArguments: tokenArgs }, hre, true);
    });
  });
  ...
  describe("via buildfile", () => {
    it("verify from build file", async () => {
      ...
      await verifyContract({ via: "buildfile", contract, buildFile: faucetTokenBuildFile, deployArgs: tokenArgs }, hre, true);
    });
  });
});
