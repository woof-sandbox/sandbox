...
export function getRelationConfig(
  ...
  throw new Error(`Must set \`relationConfigMap\` key of \`deploymentManager\` in Hardhat config to use spider.`);
}
...
export async function readAlias(contract: Contract, aliasRender: AliasRender, context: Ctx, path: Contract[]): Promise<Alias> {
