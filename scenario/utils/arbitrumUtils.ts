...
export async function estimateL2Transaction(
  ...
  const l2GasEstimateHex = await l2DeploymentManager.hre.network.provider.send("eth_estimateGas", [{ from, to, data }]);
...
export async function estimateTokenBridge(
  ...
  const l1Gateway = new Contract(l1GatewayAddress, gatewayInterface, l1DeploymentManager.hre.ethers.provider);
  const l2GatewayAddress = await l1Gateway.counterpartGateway();

  const data = gatewayInterface.encodeFunctionData("finalizeInboundTransfer", [
    token, // address _token,
    from, // address _from,
    to, // address _to,
    amount, // uint256 _amount,
    utils.defaultAbiCoder.encode(["bytes", "bytes"], ["0x", "0x"]), // bytes calldata _data
  ]);
