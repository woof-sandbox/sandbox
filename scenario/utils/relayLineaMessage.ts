...
export default async function relayLineaMessage(
  ...
  for (let messageSentEvent of messageSentEvents) {
    ...
    const aliasSetterRoleAccount = await impersonateAddress(bridgeDeploymentManager, LINEA_SETTER_ROLE_ACCOUNT);
    ...
    const relayMessageTxn = await (
      await l2MessageService.claimMessage(_from, _to, _fee, _value, constants.AddressZero, _calldata, _nonce, {
        gasPrice: 0,
        gasLimit: 10000000,
      })
    ).wait();
    ...
    if (_to === l2TokenBridge.address) {
      ...
      console.log(`[${governanceDeploymentManager.network} -> ${bridgeDeploymentManager.network}] Bridged USDC`);
