...
export default async function relayMantleMessage(
  ...
  for (let sentMessageEvent of sentMessageEvents) {
    ...
    const aliasedSigner = await impersonateAddress(bridgeDeploymentManager, applyL1ToL2Alias(mantleL1CrossDomainMessenger.address));

    await setNextBaseFeeToZero(bridgeDeploymentManager);
    const relayMessageTxn = await (
      await l2CrossDomainMessenger
        .connect(aliasedSigner)
        .relayMessage(messageNonce, sender, target, 0, 0, gasLimit, message, { gasPrice: 0, gasLimit: 7_500_000 })
    ).wait();
