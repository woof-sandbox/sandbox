...
export async function isBridgeProposal(
  ...
  switch (bridgeNetwork) {
    ...
    case "polygon": {
      const { fxRoot, RootChainManager } = await governanceDeploymentManager.getContracts();
    ...
    case "base": {
      ...
      const baseL1USDSBridge = await governanceDeploymentManager.getContractOrThrow("baseL1USDSBridge");
      const targets = openProposal.targets;
    ...
    case "optimism": {
      ...
      const opL1StandardBridge = await governanceDeploymentManager.getContractOrThrow("opL1StandardBridge");
      const targets = openProposal.targets;
    ...
    case "mantle": {
      ...
      const mantleL1StandardBridge = await governanceDeploymentManager.getContractOrThrow("mantleL1StandardBridge");
      const targets = openProposal.targets;
      const bridgeContracts = [mantleL1CrossDomainMessenger.address, mantleL1StandardBridge.address];
      return targets.some(t => bridgeContracts.includes(t));
    }
    case "scroll": {
