...
export default async function relaySonicMessage(
  ...
  for (const sentPacketEvent of sentPacketEvents) {
    ...
    const aliasedSigner = await impersonateAddress(bridgeDeploymentManager, applyL1ToL2Alias(endpointOrigin.address));

    await setNextBaseFeeToZero(bridgeDeploymentManager);

    const relayMessageTxn = await (
      await endpointDestination
        .connect(aliasedSigner)
        .lzReceive(srcEid, ethers.utils.hexlify(ethers.utils.zeroPad(endpointOrigin.address, 32)), nonce, encodedPayload, {
          gasPrice: 0,
          gasLimit: 7_500_000,
        })
    ).wait();
