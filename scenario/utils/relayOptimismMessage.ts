...
export default async function relayOptimismMessage(
  ...
  for (let sentMessageEvent of sentMessageEvents) {
    ...
    } = opL1CrossDomainMessenger.interface.parseLog(sentMessageEvent);
    const aliasedSigner = await impersonateAddress(bridgeDeploymentManager, applyL1ToL2Alias(opL1CrossDomainMessenger.address));
