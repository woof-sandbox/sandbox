...
scenario(
  ...
  async ({ comet, configurator, proxyAdmin, actors, bridgeReceiver }, context) => {
    const { albert } = actors;
    const dm = context.world.deploymentManager;
    const maticx = await dm.existing("MaticX", MATICX_ADDRESS, context.world.base.network, "contracts/ERC20.sol:ERC20");
    const maticxPricefeed = await dm.existing("MaticX:priceFeed", MATICX_PRICE_FEED_ADDRESS, context.world.base.network);

    // Allocate some tokens to Albert
    const maticxWhaleSigner = await impersonateAddress(dm, MATICX_WHALES.polygon[0]);
    ...
    await maticx.connect(maticxWhaleSigner).transfer(albert.address, exp(9000, 18).toString());
    ...
    await maticx.connect(albert.signer).approve(comet.address, supplyAmount);
    ...
    expect(await albert.getCometCollateralBalance(maticx.address)).to.be.equal(supplyAmount);
