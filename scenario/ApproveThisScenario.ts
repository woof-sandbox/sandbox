...
scenario('Comet#approveThis > allows governor to authorize and rescind authorization for Comet ERC20', {}, async ({ comet, governor, actors }, context) => {
  const { admin } = actors;
...
scenario('Comet#approveThis > allows governor to authorize and rescind authorization for non-Comet ERC20', {}, async ({ comet, governor, actors }, context) => {
  const { admin } = actors;
