...
async function testTransferFromCollateral(context: CometContext, assetNum: number): Promise<void | ContractReceipt> {
  ...
  const toTransfer = scale.toBigInt() * 50n;
  const txn = await charles.transferAssetFrom({
    src: albert.address,
    dst: betty.address,
...
scenario(
  ...
  async ({ comet, actors }, context) => {
    ...
    expectApproximately(
      await albert.getCometBaseBalance(),
      1000n * scale,
      getInterest(1000n * scale, borrowRate, BigInt(getConfigForScenario(context).interestSeconds)) + 2n
    );
    expectApproximately(
      await betty.getCometBaseBalance(),
      -1000n * scale,
    ...
    const txn = await betty.transferAssetFrom({
      src: albert.address,
      dst: betty.address,
...
scenario("Comet#transfer disallows self-transfer of base", {}, async ({ comet, actors }) => {
  const { albert } = actors;
...
scenario("Comet#transfer disallows self-transfer of collateral", {}, async ({ comet, actors }) => {
  const { albert } = actors;
...
scenario("Comet#transferFrom disallows self-transfer of collateral", {}, async ({ comet, actors }) => {
  const { albert, betty } = actors;
...
scenario("Comet#transferFrom reverts if operator not given permission", {}, async ({ comet, actors }, context) => {
  const { albert, betty } = actors;
  const baseAssetAddress = await comet.baseToken();
  const baseAsset = context.getAssetByAddress(baseAssetAddress);
  const scale = (await comet.baseScale()).toBigInt();
