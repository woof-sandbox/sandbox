...
scenario(
  ...
  async ({ comet, configurator, proxyAdmin, timelock: oldLocalTimelock, bridgeReceiver: oldBridgeReceiver }, context, world) => {
    ...
    const transferOwnershipCalldata = utils.defaultAbiCoder.encode(["address"], [newLocalTimelock.address]);
    const setGovernorCalldata = await calldata(configurator.populateTransaction.setGovernor(comet.address, newLocalTimelock.address));
    ...
    const upgradeL2GovContractsProposal = utils.defaultAbiCoder.encode(
      ...
      [
        ...
        [0, 0, 0],
        ["setGovernor(address,address)", "deployAndUpgradeTo(address,address)", "transferOwnership(address)"],
...
scenario.skip(
  ...
  async ({ comet, configurator, proxyAdmin, timelock: oldLocalTimelock, bridgeReceiver: oldBridgeReceiver }, context, world) => {
    ...
    const newBridgeReceiver = await dm.deploy<LineaBridgeReceiver, [string]>("newBridgeReceiver", "bridges/linea/LineaBridgeReceiver.sol", [
