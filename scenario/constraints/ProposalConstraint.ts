...
export async function getOpenProposals(deploymentManager: DeploymentManager, governor: IGovernorBravo): Promise<OpenProposal[]> {
  ...
  if (logs) {
    for (let log of logs) {
      ...
      if ([ProposalState.Pending, ProposalState.Active, ProposalState.Succeeded, ProposalState.Queued].includes(state)) {
        proposals.push({
          id,
          proposer,
          targets,
    ...
    return async function (ctx: T): Promise<T> {
      ...
      for (const proposal of proposals) {
        ...
        if (ctx.migrations !== undefined) {
          migrationData = ctx.migrations.find(migrationData => migrationData.lastProposal === proposal.id.toNumber());
        }
        ...
        try {
          ...
          if (isBridged) {
            await executeOpenProposalAndRelay(governanceDeploymentManager, ctx.world.deploymentManager, proposal);
          } else {
        ...
        try {
          ...
          if (migrationData) {
            await migrationData.migration.actions.verify(ctx.world.deploymentManager, govDeploymentManager, preExecutionBlockNumber);
