...
contract AssetList {
    ...
    constructor(IConfigController.CollateralToken[] memory assetConfigs) {
    ...
    function getPackedAssetInternal(
        ...
        IConfigController.CollateralToken memory token = assetConfigs[i];
        address asset = token.collateralToken;
        IConfigController.CollateralTokenConfig memory config = token.config;

        address priceFeed = config.priceFeed;
        uint8 decimals_ = config.decimals;
        uint64 borrowCF = config.borrowCollateralFactor;
        uint64 liquidateCF = config.liquidateCollateralFactor;
        uint64 liquidationF = config.liquidationFactor;
        uint128 supplyCap = config.supplyCap;
        ...
        unchecked {
            ...
            uint16 borrowCollateralFactor = uint16(borrowCF / descale);
            uint16 liquidateCollateralFactor = uint16(liquidateCF / descale);
            ...
            uint256 word_a = (uint160(asset) |
                (uint256(borrowCollateralFactor) << 160) |
                (uint256(liquidateCollateralFactor) << 176) |
                (uint256(liquidationFactor) << 192));

            uint256 word_b = (uint160(priceFeed) |
                (uint256(decimals_) << 160) |
                (uint256(scaledSupplyCap) << 168));
